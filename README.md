...
| 参数 | 作用维度 | 怎么用 |
|---|---|---|
| 判定阈值 | **颜色**：多接近背景才算背景 | 背景偏亮/偏暗，边缘还残留一圈原底色时调高 |
| 边缘羽化 | **几何**：软边过渡带多宽 | 发丝要柔和调高；要利落轮廓调低 |
| 边缘收放 | **几何**：整体平移前景边界 | 正值收缩（更彻底地去掉轮廓残留），负值扩张（保住更多发丝） |

> 这三者之所以能各管一件事，是因为把「颜色判据」与「几何宽度」解耦了。
> 早期版本把过渡带颜色上界设为阈值的固定倍数，阈值上移会同时抬高上下界、
> 净效果互相抵消，表现为滑杆几乎调不出变化。
...
**取景框可以直接拖动**：在预览的「构图」页拖动框内平移、拖角点等比缩放、滚轮微调，
界面会实时显示「取景多少像素 → 输出多少像素」。
缩放**始终锁定规格比例**，因此内核只需按框裁剪再缩放到规定像素，成品不会被拉伸变形。
...
```bash
# 固定版本（推荐用于生产）
docker pull crazyfigure/picpro:0.2.0
...
```bash
docker run -d --name picpro -p 8080:80 --restart unless-stopped crazyfigure/picpro:0.2.0
```
